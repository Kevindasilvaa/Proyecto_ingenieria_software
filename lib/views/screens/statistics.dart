...
class _StatisticsPageState extends State<StatisticsPage> {
  ...
  void _updatePieChartData() async {
    ...
    for (var transaccion in transacciones) {
      if (categoriasMontos.containsKey(transaccion.categoria_id)) {
        categoriasMontos[transaccion.categoria_id] =
            (categoriasMontos[transaccion.categoria_id] ?? 0) +
                transaccion.monto;
      } else {
    ...
    if (categoriasMontos.isEmpty) {
      ...
      for (var entry in categoriasMontos.entries) {
        ...
        Category? categoria =
            await categoryController.getCategoryById(categoriaId);
  ...
  Widget _buildLegend() {
    ...
    return Wrap(
      ...
      runSpacing: 15.0, // Espacio vertical entre las líneas
      children: _pieChartSections.map((section) {
        double percentage = (section.value / total * 100);
        return Column(
          mainAxisSize:
              MainAxisSize.min, // Ajusta la columna al tamaño de su contenido
          children: [
            Row(
              mainAxisSize:
                  MainAxisSize.min, // Ajusta la fila al tamaño de su contenido
              children: [
                Container(
                  width: 19,
                  height: 19,
                        ...
                        Center(
                          child: Row(
                            ...
                            children: [
                              ...
                              _ingreso
                                  ? Text(
                                      '+ ${userController.usuario?.monthlyIncomeBudget != null ? NumberFormat('#,##0.00', 'es_ES').format(userController.usuario?.monthlyIncomeBudget ?? 0.00) : '0.00'}',
                                      style: TextStyle(
                                        fontSize: 22,
                                        fontWeight: FontWeight.bold,
                                        color: Colors.green,
                                  ...
                                  : Text(
                                      '- ${userController.usuario?.monthlyExpenseBudget != null ? NumberFormat('#,##0.00', 'es_ES').format(userController.usuario?.monthlyExpenseBudget ?? 0.00) : '0.00'}',
                                      style: TextStyle(
                                        fontSize: 22,
                                        fontWeight: FontWeight.bold,
                                        color: Colors.red,
                        ...
                        Center(
                          child: Row(
                            ...
                            children: [
                              _ingreso
                                  ? Text(
                                      '+ ${_transaccionesFiltradas.isNotEmpty ? NumberFormat('#,##0.00', 'es_ES').format(_transaccionesFiltradas.fold(0.0, (sum, transaccion) => sum + transaccion.monto)) : '0.00'}',
                                      style: TextStyle(
                                        fontSize: 22,
                                        fontWeight: FontWeight.bold,
                                        color: Colors.green,
